...
const (
	apiEndpoint = "https://api.squadcast.com"
)
...
type Client struct {
	...
	// squadcast API. You can use either *http.Client here, or your own
	// implementation.
...
func (c *Client) do(method, path string, body io.Reader, headers *map[string]string) (*http.Response, error) {
	...
	req.Header.Set("Accept", "application/vnd.squadcast+json;version=2")
	if headers != nil {
		for k, v := range *headers {
			req.Header.Set(k, v)
		}
	}
	req.Header.Set("User-Agent", "go-squadcast/"+Version)
